  ...
  const selected = useMemo(() => {
    ...
    return (
      projects?.find((project) => project.slug === slug) || {
        ...
        slug: '/',
        domain: '',
        logo: session?.user?.image || `https://api.dicebear.com/7.x/pixel-art/svg?seed=${session?.user?.email}.svg`
      }
    );
          ...
          <div className="flex justify-start items-center space-x-3">
            {selected.slug === '/' ? (
              <img
                ...
                src={session?.user?.image || `https://api.dicebear.com/7.x/pixel-art/svg?seed=${session?.user?.email}.svg`}
