      ...
      <div className="rounded-lg border border-gray-200 bg-white">
        <div className="flex items-center justify-between p-10">
          <div className="flex flex-col space-y-3">
            ...
            <p className="text-sm text-gray-500">Teammates or friends that have access to this project.</p>
          </div>
          <button
        ...
        <div className="flex space-x-3 border-b border-gray-200 px-7">
          {tabs.map((tab) => (
            <div className={`${tab === currentTab ? 'border-black' : 'border-transparent'} border-b py-1`}>
              <button
              ...
              users.map(({ name, email, image, joinedAt }) => (
                <div className="flex items-center justify-between space-x-3 px-8 py-3">
                  <div className="flex items-center space-x-3">
                    <img
                      src={image || `https://api.dicebear.com/7.x/pixel-art/svg?seed=${email}.svg`}
                      alt={email}
                      width={40}
                      height={40}
                    ...
                    <div className="flex flex-col">
                      <h3 className="text-sm font-medium">{currentTab === 'Members' ? name || 'Anonymous' : 'Invitation Sent'}</h3>
